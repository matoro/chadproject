...
+ Waiting to implement *health and ammo bars* as part of the *UI* layer. When initially creating objects, we decided to include bars alongside other objects such as the player character, enemies, and bullets. Bars currently seem spit between the object and UI layer. They are created and updated in a UI function, but are objects. In hindsight, it makes more sense to handle bars soley in the UI layer. 
